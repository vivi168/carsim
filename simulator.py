...
class Wheel(ActsAsDict):
  def __init__(self, section_width, aspect_ratio, rim_diameter, mu, pressure):
    self.radius = ((rim_diameter * Constant.CM_PER_INCH / 2) +
                   (section_width / 10 * aspect_ratio / 100)) / 100  # m
...
class Car:
  def __init__(self, mass, C_d, A, launch_rpm, redline_rpm, engine,
               transmission, wheel):
  ...
  def C_rr(self):
    return 0.005 + (1 / self.wheel.tire_pressure) * (0.01 + 0.0095 *
                                                     (self.v * 3.6 / 100)**2)
...
class Logger:
  def __init__(self,
               speed_points=[60, 100, 120, 200],
               position_points=[0.25, 0.5, 1]):
  ...
  def __str__(self):
    ...
    for p in self.speed_points:
      if self.speed_summary[p]:
        summary.append('{:<12} {:<6.2f} seconds'.format(
            "0-%s mph" % p, self.speed_summary[p]))

    for p in self.position_points:
      if self.position_summary[p]:
        summary.append(len(summary[0]) * '-')
        summary.append('{:<12} {:<6.2f} seconds'.format(
            str(fractions.Fraction(p)) + " mi", self.position_summary[p][0]))
        summary.append('{:<12} {:<6.2f} mph'.format(
            str(fractions.Fraction(p)) + " mi spd",
            self.position_summary[p][1]))
...
def main(car_file):
  ...
  result = [[
      "time", "RPM", "gear", "T_eng", "T_wheel", "F_wheel", "F_wheel_max",
      "F_drive", "F_drag", "F_rr", "F_net", "a", "v", "s"
  ]]

  while t < t_max:
    car.engine.current_rpm = (
        car.v / car.wheel.radius) * car.transmission.get_ratio() * 60 / (
            2 * Constant.PI)
    ...
    result.append([
        t, car.engine.current_rpm, car.transmission.current_gear, T_eng,
        T_wheel, F_wheel, F_wheel_max, F_drive, F_drag, F_rr, F_net, car.a,
        car.v, car.s
    ])
